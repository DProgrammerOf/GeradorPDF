...
try {
    ...
    if($tipoReq === 'POST'){
      $pathFileLogo =  "upload/empresa/logo/" . basename($_FILES["imageLogo"]["name"]);
      $pathFileCarimbo = "upload/empresa/carimbo/"  . basename($_FILES["imageCarimbo"]["name"]);
      ...
      if ( move_uploaded_file($_FILES["imageLogo"]["tmp_name"], $pathFileLogo) )
        if ( move_uploaded_file($_FILES["imageCarimbo"]["tmp_name"], $pathFileCarimbo) )
          {
            $updateEmpresa = $conexaoBanco->prepare('UPDATE empresas SET logomarca = :pathlogo, carimbo = :pathcarimbo WHERE id = :id');
